...
export default async function AdminGuestsPage() {
  ...
  const [locationsResult, guestsResult] = await Promise.all([
    ...
    supabase.from("visits").select("id, guest_name, guest_organization, guest_host_name, guest_purpose, check_in_at, locations(name)").eq("person_type", "GUEST").is("check_out_at", null).order("check_in_at", { ascending: false }).limit(200),
  ]);
