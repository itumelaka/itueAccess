...
export default async function AdminLocationsPage() {
  ...
  return <main><header className="admin-page-header"><div><p className="admin-kicker">Konfigurasi</p><h1>Lokasi & kod QR</h1><p>Setiap kod membuka halaman daftar masuk lokasi berkenaan.</p></div></header>
    ...
    <section className="qr-grid">{rows.map((location) => <article className={`qr-card ${location.is_active ? "" : "qr-card--inactive"}`} key={location.id}><div className="qr-card__top"><div><h2>{location.name}</h2><code>{location.code}</code></div><span className={`status status--${location.is_active ? "active" : "suspended"}`}>{location.is_active ? "AKTIF" : "TIDAK AKTIF"}</span></div><Image src={location.qr} width={220} height={220} alt={`Kod QR ${location.name}`} unoptimized/><a href={location.scanUrl}>{location.scanUrl}</a><div className="qr-card__actions"><a className="admin-primary" href={location.qrDownloadUrl} download={location.qrFileName}>Muat turun QR</a><form action={toggleLocation}><input type="hidden" name="locationId" value={location.id}/><input type="hidden" name="isActive" value={String(location.is_active)}/><button className="admin-secondary" type="submit">{location.is_active ? "Nyahaktifkan" : "Aktifkan semula"}</button></form></div></article>)}</section>
  </main>;
}
