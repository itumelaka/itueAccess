...
export default async function AdminHistoryPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; location?: string; type?: string }> }) {
  ...
  let query = supabase.from("visits").select("id, person_type, guest_name, guest_host_name, check_in_at, check_out_at, locations(name), profiles!visits_profile_id_fkey(display_name)").gte("check_in_at", start.toISOString()).lte("check_in_at", end.toISOString()).order("check_in_at", { ascending: false }).limit(500);
  ...
  return <main><header className="admin-page-header"><div><p className="admin-kicker">Audit operasi</p><h1>Sejarah akses</h1><p>Carian dihadkan kepada maksimum 90 hari dan 500 rekod.</p></div></header>
    ...
    <section className="admin-panel table-wrap"><table className="admin-table"><thead><tr><th>Nama</th><th>Jenis</th><th>Pegawai ditemui</th><th>Lokasi</th><th>Masuk</th><th>Keluar</th></tr></thead><tbody>{visitsResult.data?.map((visit) => <tr key={visit.id}><td>{visit.person_type === "GUEST" ? visit.guest_name : visit.profiles?.display_name}</td><td>{visit.person_type === "GUEST" ? "Tetamu" : "Pengguna"}</td><td>{visit.person_type === "GUEST" ? (visit.guest_host_name ?? "—") : "—"}</td><td>{visit.locations.name}</td><td>{new Date(visit.check_in_at).toLocaleString("ms-MY", { timeZone: "Asia/Kuala_Lumpur" })}</td><td>{visit.check_out_at ? new Date(visit.check_out_at).toLocaleString("ms-MY", { timeZone: "Asia/Kuala_Lumpur" }) : <span className="status status--active">Di dalam</span>}</td></tr>)}</tbody></table>{!visitsResult.data?.length && <p className="empty-state">Tiada rekod untuk penapis ini.</p>}</section>
  </main>;
}
