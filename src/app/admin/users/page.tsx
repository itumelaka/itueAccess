...
export default async function AdminUsersPage() {
  ...
  return <main><header className="admin-page-header"><div><p className="admin-kicker">Kawalan akses</p><h1>Pengguna</h1><p>Luluskan akaun baharu dan urus status pengguna.</p></div><span className="count-pill">{pending.length} menunggu</span></header>
    <section className="admin-panel"><div className="panel-heading"><h2>Menunggu kelulusan</h2></div><div className="admin-list">
      {pending.length ? pending.map((profile) => <article className="user-row" key={profile.id}><div><strong>{profile.display_name}</strong><small>{profile.email}</small></div><div className="user-actions"><form action={approveUser} className="inline-form"><input type="hidden" name="profileId" value={profile.id}/><select name="category" defaultValue="" required><option value="" disabled>Pilih kategori</option><option value="STAFF">Staf</option><option value="PELATIH">Pelatih</option></select><button className="admin-primary" type="submit">Luluskan</button></form><form action={rejectUser}><input type="hidden" name="profileId" value={profile.id}/><button className="admin-danger" type="submit">Ditolak</button></form></div></article>) : <p className="empty-state">Tiada akaun menunggu kelulusan.</p>}
    </div></section>
