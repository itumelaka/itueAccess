...
export function StatusCard({ visit }: StatusCardProps) {
  return (
    <section className={`rounded-3xl p-6 ${visit ? "bg-emerald-50 text-emerald-950" : "bg-slate-100 text-slate-900"}`}>
      ...
      {visit ? (
        <><h2 className="mt-2 text-2xl font-bold">Sedang berada di {visit.locationName}</h2><p className="mt-2">Masuk pada {new Date(visit.checkInAt).toLocaleString("ms-MY")}</p></>
      ) : (
